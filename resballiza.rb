...
class MyGame < Gosu::Window
	...
	def button_down (id)
		#
		if id == Gosu::Button::KbP  # Pause
			if @pause == false
				#
				if @score >= 1  #This will disable pause if game intro is running
				@pause = true 
				end
				#
				if @running == false
		...
		#
		if id == Gosu::Button::KbSpace  # Start Game Screen
			if @start == false and @animation_time >= 350  #This will work only when game intro is finished
				@start = true
	...
	#
	#
	def update  # check if key is pressed
		#
		...
		if @start == true
			...
			if @pause == false
				...
				if @running
					...
					#
					@balls.each {|ball| ball.y == 0 and @ballsfall_sample.play}  #This was very hard to figure out by myself
					#
					...
					if button_down? Gosu::Button::KbEscape
						...
						@score = 1 		# If I made @score = 0 the game intro screen would appear temporarly when game is restarted. 
